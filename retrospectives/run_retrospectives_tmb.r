...
# Run retrospective analysis for TMB copy of BASA

# CL Roberts

# model currently only runs for n.peel < 3 :(
...
for(i in 1:n.peels) {
    ...
    # create retro subdirectories
    dir_retro_i <- here::here(dir_retro, paste0("basa-tmb-", i))
    if(!dir.exists(dir_retro_i)){
        dir.create(dir_retro_i)
    }

    # copy model to subdirs
    file.copy(here::here(dir_model, "PWS_ASA_tmb.cpp"), dir_retro_i)
    file.copy(here::here(dir_model, "PWS_ASA_tmb.dll"), dir_retro_i)
    file.copy(here::here(dir_model, "PWS_ASA_tmb.o"), dir_retro_i)
    ...
    # remember change when to ESS's are implemented!!!
    file.copy(here::here(dir_model, "PWS_ASA(ESS).ctl"), dir_retro_i) 

    # peel off i number of years in data
    dat.files <- read.data.files(dir_model)
    write_retro_dat(dat.files, i, dir_retro_i)

    # ESS's
    PWS_ASA_ESS <- read.data.files(dir_retro_i)$"PWS_ASA_ESS.ctl" 
    # model data
    PWS_ASA <- read.data.files(dir_retro_i)$"PWS_ASA.dat"  
    # recruitment and natural mortality deviate formulations
    PWS_ASA_covariate <- read.data.files(dir_retro_i)$"PWS_ASA_covariate.ctl"
    # age composition sample sizes   
    agecomp_samp_sizes <- read.data.files(dir_retro_i)$"agecomp_samp_sizes.txt"
    # disease data
    PWS_ASA_disease <- read.data.files(dir_retro_i)$"PWS_ASA_disease.dat"
    ...
    dyn.load(dynlib(here::here(dir_retro_i, "PWS_ASA_tmb")))
    ...
    lower_init <- c(annual_age0devs = rep(-.1, Y-1), 
                    ...
                    VHSV_age3_4_mort_93 = 0.1, ICH_age5_8_mort_93 = 0.1, 
                    mat_age3 = 0.4, mat_age4 = 0.8,     
                    seine_selex_alpha = 3, seine_selex_beta = 4,
                    milt_add_var = 0.3, 
                    adfg_hydro_add_var = 0.1, pwssc_hydro_add_var = 0.1,
    ...
    upper_init <- c(annual_age0devs = rep(.1, Y-1), 
                    ...
                    VHSV_age3_4_mort_93 = .3, ICH_age5_8_mort_93 = .3, 
                    mat_age3 = 0.9, mat_age4 = .9, 
    ...
    dir_mcmc_tmb <- here::here(dir_retro_i, "mcmc_out_tmb")
    ...
    # note: a placeholder value of 1 is used for the forecast
    write.table(cbind(t(do.call(cbind, Btilde_y)), 1), sep = ",", 
