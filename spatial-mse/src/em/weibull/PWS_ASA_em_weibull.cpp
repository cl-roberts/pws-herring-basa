...
Type dgengamma(Type x, Type mean, Type sigma, Type Q, int give_log=0){
    ...
    Type qi = pow(Q, -2);
    Type qw = Q*w;
    Type loglik = -log(sigma*x) + log(abs_q)*(1-2*qi) + qi*(qw-exp(qw)) - lgamma(qi);
    if(give_log) {
        return loglik;
    } else {
        return exp(loglik);
...
Type objective_function<Type>::operator() ()
{
    ...
    // ---- for gengamma em ---- //
    // PARAMETER(Q);                       // general gamma shape parameter
    ...
    for (int y = 0; y < nyr; y++) {
        if (mdm(y) == -9) continue;
        L6 -= dgengamma(mdm(y), That_y(y), milt_add_var, Type(1.0), true);
        n_mdm += 1;
    }
