...
Type objective_function<Type>::operator() ()
{
    ...
    PARAMETER(log_lambda);                    // fraction that move from KI to PWS
    ...
    Type PWS_lambda = 0.0;
    PWS_lambda = 1 / (1+exp(-log_lambda));
    Type KI_lambda = PWS_lambda;
    ...
    L10 /= 2*pow(milt_add_var, 2);
    L10 += n_KI_mdm*log(milt_add_var);
