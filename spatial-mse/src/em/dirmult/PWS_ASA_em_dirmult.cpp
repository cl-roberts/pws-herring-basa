...
Type ddirmultinom(vector<Type> obs, vector<Type> alpha, int do_log) {
    ...
    Type N = obs.sum();
    Type phi=sum(alpha);
    Type loglik = lgamma(N + 1.0) + lgamma(phi) - lgamma(N + phi);
    for(int a = 0; a < dim; a++) {
        loglik += -lgamma(obs(a) + 1.0) + lgamma(obs(a) + alpha(a)) - lgamma(alpha(a));
    }
    if(do_log == 1) return loglik;
...
Type objective_function<Type>::operator() ()
{
    ...
    for (int y = 0; y < nyr; y++) {
        ...
        if (y < index_1984) {
            ...
            for (int a = 3; a < plus_group_index; a++) {
                est_seine_vector(a-3) = seine_selex(a)*N_y_a(y, a);
                obs_seine_vector(a-3) = seine_sample_size(y)*seine_age_comp(y, a);
            }
            est_seine_vector(plus_group_index-3) = Ntilde_y_a(y, plus_group_index);
            obs_seine_vector(plus_group_index-3) = seine_sample_size(y)*seine_age_comp.row(y).tail(nage-plus_group_index).sum();

            L1_years(y) = ddirmultinom(obs_seine_vector, est_seine_vector, 1);
    ...
    for (int y = 0; y < nyr; y++) {
        ...
        if (y < index_1984) {
            ...
            for (int a = 3; a < plus_group_index; a++) {
                ...
                est_spawners_vector(a-3) = Ntilde_y_a(y, a);
            }
            est_spawners_vector(plus_group_index-3) = Ntilde_y_a(y, plus_group_index);
            obs_spawners_vector(plus_group_index-3) = spawn_sample_size(y)*spawn_age_comp.row(y).tail(nage-plus_group_index).sum();

            L2_years(y) = ddirmultinom(obs_spawners_vector, est_spawners_vector, 1);
