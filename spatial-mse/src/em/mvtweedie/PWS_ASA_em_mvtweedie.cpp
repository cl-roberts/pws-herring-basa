...
Type dmvtweedie(vector<Type> x, vector<Type> prob, Type phi, Type power, int give_log=0) {
    int nage = x.size();
    vector<Type> p_exp(nage);
    p_exp = prob;
    p_exp /= sum(p_exp);
    Type Ntotal = asDouble(sum(x)); 
    Type loglik = 0;
    for (int c=0; c<nage; c++) {
        loglik += dtweedie(x(c), p_exp(c)*Ntotal, phi, power, true);
    }
      if(give_log) return loglik; else return exp(loglik);
}
...
Type objective_function<Type>::operator() ()
{
    ...
    // ---- parameters for dmtweedie em ---- //
    PARAMETER(seine_log_phi);                     // tweedie phi parameter
    PARAMETER(seine_psi_star);                   // tweedie power parameter
    PARAMETER(spawn_log_phi);                     // tweedie phi parameter
    PARAMETER(spawn_psi_star);                   // tweedie power parameter
    ...
    int nageovs = beta_mortality.size();
    
    matrix<Type> disease_covs_calc(nyr, nageovs);
    ...
    vector<Type> mean_disease_cov(nageovs);              // disease covariate for forecast
    ...
    Type seine_power = 1 + (1 / (1+exp(-seine_psi_star)));
    ...
    for (int y = 0; y < nyr; y++) {
        ...
        if (seine_ess(y) == -9) continue;               // skip years with no survey

        if (y < index_1984) {

            int plus_group_index = 5 + y;                   // index of plus group 

            vector<Type> obs_seine_vector(plus_group_index+1-3);
            obs_seine_vector.setZero();
            vector<Type> est_seine_vector(plus_group_index+1-3);
            est_seine_vector.setZero();

            for (int a = 3; a < plus_group_index; a++) {
                obs_seine_vector(a-3) = seine_sample_size(y)*seine_age_comp(y, a);
                est_seine_vector(a-3) = seine_age_comp_est(y, a);
            }
            est_seine_vector(plus_group_index-3) = seine_age_comp_est(y, plus_group_index);
            obs_seine_vector(plus_group_index-3) = seine_sample_size(y)*seine_age_comp.row(y).tail(nage-plus_group_index).sum();

            L1_years(y) = dmvtweedie(obs_seine_vector, est_seine_vector, exp(seine_log_phi), seine_power, 1);

        } else if (y >= index_1984) {

            vector<Type> obs_seine_vector(nage-3);
            obs_seine_vector.setZero();
            vector<Type> est_seine_vector(nage-3);
            est_seine_vector.setZero();            
    ...
    Type spawn_power = 1 + (1 / (1+exp(-spawn_psi_star)));
    ...
    for (int y = 0; y < nyr; y++) {
        ...
        if (y < index_1984) {
            ...
            vector<Type> obs_spawners_vector(plus_group_index+1-3);
            obs_spawners_vector.setZero();
            vector<Type> est_spawners_vector(plus_group_index+1-3);
            est_spawners_vector.setZero();

            for (int a = 3; a < plus_group_index; a++) {
                obs_spawners_vector(a-3) = spawn_sample_size(y)*spawn_age_comp(y, a);
                est_spawners_vector(a-3) = spawn_age_comp_est(y, a);
            }
            est_spawners_vector(plus_group_index-3) = spawn_age_comp_est(y, plus_group_index);
            obs_spawners_vector(plus_group_index-3) = spawn_sample_size(y)*spawn_age_comp.row(y).tail(nage-plus_group_index).sum();

            L2_years(y) = dmvtweedie(obs_spawners_vector, est_spawners_vector, exp(spawn_log_phi), spawn_power, 1);

        } else if (y >= index_1984) {

            vector<Type> obs_spawners_vector(nage-3);
            obs_spawners_vector.setZero();
            vector<Type> est_spawners_vector(nage-3);
            est_spawners_vector.setZero();            
    ...
    // priors -= dnorm(phi, Type(3.0), Type(1.0), true);
    // priors -= dnorm(power, Type(1.45), Type(0.25), true);
