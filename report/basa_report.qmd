...
In this report, we present the fits and estimates from the `r curr.year-1L` BASA model. Note that `r curr.year-1L` BASA models the state of the herring stock up to, and including, autumn `r curr.year-1L`. We project winter mortality to estimate the stock's pre-fishery biomass for `r curr.year` but do not make a recruitment forecast. Persistent low levels of biomass and recruitment since the early 1990s continue to preclude consideration of reopening of fisheries under the current harvest strategy, and thus forecasts are not conducted. At present, BASA is primarily used to estimate spawning biomass and recruitment up to the most recent year with data and is used as a research tool to investigate hypotheses and evaluate alternative models.
...
ssb_est |>
	...
	fontsize(j=c(3, 4, 6, 7, 10, 11), size=6, part="body") |>	
