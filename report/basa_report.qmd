...
# 	prettyNum(x, big.mark = ",", big.interval = 4L)
# })
```
...
# `r curr.year-1` BASA Summary
To run the `r curr.year-1` BASA model, the key software and versions used include:

* AD Model Builder v. 13.2
* R                v. `r paste(version$major, version$minor, sep = ".")`
* adnuts           v. `r packageVersion("adnuts")`
...
Posterior predictive intervals for the age composition data mostly show good fits, except for the age-3 classes in 1987 and 1998 (@fig-age-comps) as well as the age-4 and -7 class in `r curr.year-1`. The model misfit in `r curr.year-1` could be due to a reduced sample size from the ASL survey being used. Estimates of the age-3 and age-4 year classes support strong 2019 and 2020 cohort. The median spawning biomass estimate in `r curr.year-1` was approximately 28,700 metric tons which is above ADF&G's lower cut-off for fishing (@tbl-management-outputs). Additionally, uncertainty in this estimate indicates there was a 5% probability that `r curr.year-1` spawning biomass was below this lower cutoff (@tbl-management-outputs).
...
# `r curr.year-1` BASA Diagnostics
...
5-year retrospectives for the `r curr.year-1` BASA model. Peels show 
