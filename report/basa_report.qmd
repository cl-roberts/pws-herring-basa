...
In this report, we present the fits and estimates from the `r curr.year-1L` BASA model. Note that `r curr.year-1L` BASA models the state of the herring stock up to autumn `r curr.year-1L`. To estimate `r curr.year` spawning biomass we projected winter mortality, assuming no fall fishery, to estimate the stock's pre-fishery biomass for `r curr.year` immediately prior to recruitment. Thus, no `r curr.year` recruitment forecast is made. For all other years, we define spawning biomass as the spring biomass of mature herring immediately after recruitment and before spring fisheries.  At present, BASA is primarily used to estimate spawning biomass and recruitment up to the most recent year with data and is used as a research tool to investigate hypotheses and evaluate alternative models.
